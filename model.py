...
class Player(SQLModel, table=True):
    ...
    positions: list["SnakePosition"] = Relationship(
        back_populates="player",
        sa_relationship_kwargs={"order_by": "desc(SnakePosition.created_at)"},
    )
...
def remove_old_positions(game: Game, session: Session):
    """
    Remove the oldest position for each player as long as it has more than 1 position
    """
    for player in game.players:
        positions = session.exec(
            select(SnakePosition)
            .where(SnakePosition.player_id == player.id)
            .order_by(SnakePosition.created_at.desc())
        ).all()
        if len(positions) > 1:
...
def generate_new_positions(game: Game, session: Session):
    ...
    for player in game.players:
        x, y = move_position(
            player.positions[0].x, player.positions[0].y, player.direction
        )
    ...
    new_positions = [
        position for position in new_positions if position_by_coords[pos_tuple] == 1
    ]
